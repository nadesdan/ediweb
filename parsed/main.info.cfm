...
<cfset myFusebox.thisFuseaction = "info">
<cfset itemInfo = application.ediQuery.qryItemSummary(#attributes.shortName#,#attributes.status#,#attributes.batchid#) >
<!--- do action="v.info" --->
<cfset myFusebox.thisCircuit = "v">
<cfsavecontent variable="pageContent">
<cfset xfa.autoProcess = "main.autoProcess" />
<cfset xfa.viewStoreInfo = "main.storeinfo" />
<cftry>
<cfoutput><cfinclude template="../views/dsp_info.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 12 and right(cfcatch.MissingFileName,12) is "dsp_info.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_info.cfm in circuit v which does not exist (from fuseaction v.info).">
